...
bool cardswitch()
{
	...
	times++;
	power=13*13*13;
	cards[0]=(times-times%power)/power;
	cards[1]=(times-cards[0]*power-times%(power/13))/(power/13);
	cards[2]=(times-cards[0]*power-cards[1]*power/13-times%(power/13/13))/(power/13/13);
	cards[3]=(times-cards[0]*power-cards[1]*power/13-cards[2]*power/13/13-times%(power/13/13/13))/(power/13/13/13);
	cards[0]++;
	cards[1]++;
	cards[2]++;
	cards[3]++;
...
void waiting()
{
	int seconds=((float)(clock()-t1)/done*(28561-done))/CLOCKS_PER_SEC;
	int minutes=(seconds-seconds%60)/60;
	seconds=seconds-minutes*60;
	int i;
	float percent=100*(float)done/28561;
...
int paranthesis(int solutions)
{
	...
	while(more==true)
	{
		...
		if(value>=24-0.01&&value<=24+0.01)
		{
			displaying();
			solutions++;
			fprintf(target, "%4d: ((%2d %c %2d) %c %2d) %c %2d  = %d\n", solutions, (int)temp[0], operations[0], (int)temp[1], operations[1], (int)temp[2], operations[2], (int)temp[3], (int)value);
			return solutions;
		}
